...
const REDIRECT_URI = typeof window !== "undefined" ? window.location.href.split("?")[0].split("#")[0] : "";
...
export default function App() {
  ...
  const handleAuth = () => {
    ...
    window.location.href = getSpotifyAuthUrl(clientId.trim());
  };
  ...
  const handleFetch = useCallback(async () => {
    ...
    if (!id) {
      setError("Couldn't parse a playlist ID from that URL.");
      return;
    }
    setLoading(true);
    try {
  ...
  const openBeatport = (idx, artist, title) => {
    const url = buildBeatportUrl(artist, title);
    ...
    <div style={styles.root}>
      ...
      <div style={styles.container}>
        ...
        {!token ? (
          <div style={styles.card}>
            ...
            <p style={styles.cardDesc}>
              ...
              — set the Redirect URI to this page's URL, then paste your Client ID below.
            </p>
          ...
          <>
            ...
            {playlist && (
              <div style={styles.playlistMeta}>
                {playlist.images?.[0] && (
                  <img src={playlist.images[0].url} style={styles.playlistArt} alt="cover" />
                )}
                <div>
                  <div style={styles.playlistName}>{playlist.name}</div>
                  <div style={styles.playlistSub}>
                  ...
                  <div style={styles.progressRow}>
                    <div style={styles.progressBar}>
                      <div
                        style={{
                          ...styles.progressFill,
                          width: tracks.length ? `${(searchedCount / tracks.length) * 100}%` : "0%",
                        }}
                      />
                    </div>
                    <span style={styles.progressLabel}>
                      {searchedCount}/{tracks.length} searched
                    </span>
                  </div>
                </div>
              </div>
            ...
            {tracks.length > 0 && (
              <div style={styles.controls}>
                ...
                <button style={styles.openAllBtn} onClick={openAll}>
                  ⚡ Open All in Beatport
                </button>
              </div>
            )}
                    ...
                    >
                      ...
                      <div style={styles.trackActions}>
                        ...
                        <button
                          ...
                          onClick={() => openBeatport(realIdx, artist, title)}
                        >
                          Search Beatport →
                        </button>
                      </div>
        ...
        <footer style={styles.footer}>
          Beatport links open as searches — no official Beatport API is used.
        </footer>
      </div>
...
const styles = {
  ...
  trackRow: {
    ...
    transition: "border-left-color 0.3s ease, background 0.2s",
  },
  trackLeft: {
    display: "flex",
    alignItems: "center",
